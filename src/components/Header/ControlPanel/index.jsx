...
const ControlPanelButton = ({onClick}) => {
    const onControlPanelClick = () => {
        setSpeed(8);
        playSegments([0, controlPanelIconOptions.animationData.op], true);
    }
    const {View, playSegments, setSpeed} = useLottie({...controlPanelIconOptions, onComplete: onClick});

    return (
        <button className="  -mr-3" onClick={onControlPanelClick}>
...
export const ControlPanel = () => {
    ...
    return (
        <>
            ...
            {/*    <ControlPanelButton />*/}
            {/*</button>*/}
            <ControlPanelButton onClick={() => setIsOpen(true)} />
